...
import { markDisputeServiceRendered } from "@/lib/services/disputeService";
...
export async function POST(
  ...
  try {
    ...
    const returnTo = sanitizeRelativePath(
      (formData?.get("returnTo") as string | null) ?? "/admin",
      "/admin",
    );

    await markDisputeServiceRendered(disputeId, admin.email);
    const url = new URL(returnTo, request.url);
    url.searchParams.set("service", "rendered");
