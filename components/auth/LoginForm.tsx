...
export function LoginForm({ email = "", hasError = false, next = "" }: LoginFormProps) {
  return (
    <form action="/api/auth/login" method="post" className="space-y-5">
      ...
      {hasError ? (
        <div className="rounded-[1rem] border border-rose-400/20 bg-rose-500/10 px-4 py-4 text-sm leading-7 text-rose-200">
          We could not sign you in with that email and password. Check your credentials and try again.
        </div>
      ) : null}
