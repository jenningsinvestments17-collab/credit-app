...
export function AdminPipelineBoard({ model }: { model: CommandCenterModel }) {
  return (
          ...
          {model.sections.map((section) => (
            <div key={section.id} className="rounded-[1.25rem] border border-white/10 bg-white/[0.04] p-4">
              ...
              <div className="mt-4 grid gap-3">
                {section.leads.length ? (
                  section.leads.slice(0, 8).map((lead) => (
                    <Link
                      key={lead.id}
                    ...
                    >
                      <p className="font-semibold uppercase tracking-[0.08em] text-white">{lead.fullName}</p>
                      <p className="mt-1 text-xs uppercase tracking-[0.18em] text-zinc-500">
                        {lead.aiReviewStatus.replaceAll("_", " ")}
                      </p>
                    </Link>
                  ))
                ) : (
