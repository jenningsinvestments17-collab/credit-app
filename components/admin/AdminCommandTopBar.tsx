...
export function AdminCommandTopBar({
  query,
  notificationCount,
  adminEmail,
  adminRole,
        ...
        <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:items-center">
          ...
          <div className="flex min-h-12 items-center gap-3 rounded-[1rem] border border-white/10 bg-white/[0.06] px-4 text-sm text-zinc-300">
            ...
            <div className="min-w-0">
              <p className="truncate text-white">{adminEmail}</p>
              <p className="text-[11px] uppercase tracking-[0.18em] text-zinc-500">{adminRole.replaceAll("_", " ")}</p>
            </div>
          </div>
        </div>
      </div>
